...
It's developed by GNOME as part of the GTK+ project. The developers decided to separate code from GTK+ not used for GUIs, so GLib became a separate bundle. It allows developers not using GUI related parts of GTK+ to use the library without overhead from depending on the full library. Probably.

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?

They add extra features that the core C library doesn't have. Also allows consistent behavior on multiple operating systems
